  ...
  try {
    ...
    const raceRes = await client.query<{
      ...
      SELECT
        ...
        COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rr2.eligible_for_prize = true)::text AS wins,
        COUNT(rr2.id) FILTER (WHERE rr2.rank <= 3)::text                               AS podiums,
        COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rr2.eligible_for_prize = true AND rm.entry_type = 'free')::text AS free_wins,
        COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rr2.eligible_for_prize = true AND rm.entry_type = 'coins_battle')::text AS cb_wins,
        COUNT(DISTINCT rp.race_room_id) FILTER (WHERE rm.entry_type = 'coins_battle')::text AS cb_joined,
        COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rr2.eligible_for_prize = true AND rm.is_private = false AND rm.type NOT IN ('country_battle','friends'))::text AS pub_wins,
        COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rr2.eligible_for_prize = true AND rm.is_private = true)::text AS priv_wins,
        ...
        COUNT(rr2.id) FILTER (WHERE rm.type = 'sponsored' AND rr2.eligible_for_prize = true)::text AS sponsored_wins,
