...
describe("race win counters", () => {
  it("counts only 1st place as race wins while keeping top-3 podium stats separate", () => {
    ...
    expect(profileRoute).toContain("return rank === 1");
  ...
  it("aligns title race-win metrics with first-place wins and podiums with top-3", () => {
    const titleEvaluation = readFileSync("src/lib/titleEvaluation.ts", "utf8");

    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank = 1)::text                                AS wins");
    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank <= 3)::text                               AS podiums");
    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rm.entry_type = 'free')::text     AS free_wins");
    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rm.entry_type = 'coins_battle')::text AS cb_wins");
    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rm.is_private = false");
    expect(titleEvaluation).toContain("COUNT(rr2.id) FILTER (WHERE rr2.rank = 1 AND rm.is_private = true)::text");
  });
});
